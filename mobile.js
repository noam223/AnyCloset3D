...
function updateMobileCellSheetState() {
    ...
    const _setHeight = (inputId) => {
        ...
        if (heightInput && state.selection.rows.length > 0) {
            ...
            heightInput.title = 'גובה תא בס״מ';
...
function _applyMobileCellHeight(delta) {
    ...
    if (typeof window._syncDrawerCompAfterHeight === 'function') {
        window._syncDrawerCompAfterHeight(col, r);
    }
...
function bindMobileUI() {
    ...
    if (cellHeightInput) {
        cellHeightInput.addEventListener('change', () => {
            ...
            const col = state.columns[c];
            if (!col) return;
            const desiredH = parseFloat(cellHeightInput.value);
...
var _DIMS_CONFIG = {
    ...
    height: { min: 100, max: 370, step: 1, key: 'height',      stateKey: 'globalHeight' },
